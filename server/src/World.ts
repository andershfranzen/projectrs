...
export class World {
  ...
  private hasClearObjectInteractionEdge(
    ...
    if (allowAuthoredNonAdjacentTile && (usesMapAuthoredObjectCollision(obj.def) || !obj.def.blocking)) return true;
  ...
  sendEquipment(player: Player): void {
    ...
    this.sendEquipmentBonuses(player);
  }

  private sendEquipmentBonuses(player: Player): void {
    const itemDefs = this.data?.itemDefs ?? new Map<number, ItemDef>();
    const bonuses = player.computeBonuses(itemDefs);
    const values = COMBAT_BONUS_WIRE_KEYS.map((key) => bonuses[key]);
    this.sendToPlayer(player, ServerOpcode.PLAYER_EQUIPMENT_BONUSES, ...values);
  }
